...
class CPlotDriver:
    ...
    def on_scroll(self, event):
        """
        鼠标滚轮缩放：同时自动调整Y轴范围（类似TradingView）
        """
        ...
        scale_factor = 0.2 # 缩放速率
        
        if event.button == 'up':
            new_range = x_range * (1 - scale_factor)
        else:
            new_range = x_range * (1 + scale_factor)
            
        mouse_x_ratio = (event.xdata - x_min) / x_range
        new_x_min = event.xdata - new_range * mouse_x_ratio
        new_x_max = new_x_min + new_range
        ...
        # --- 2. 处理 Y 轴自动适应 (Auto-Scale) ---
        # 找到当前操作的是哪个级别的图
        current_meta = None
        for lv, axes_list in self.axes_dict.items():
            # axes_list[0] 是 K线图，axes_list[1] 是 MACD
            if ax == axes_list[0]: 
                idx = self.lv_lst.index(lv)
                current_meta = self.plot_metas[idx]
                break
        
        # 如果是在K线图上滚动，则计算新的高低点
        if current_meta:
            # 计算当前视野内的 K 线索引范围
            i_start = int(max(0, new_x_min))
            i_end = int(min(current_meta.klu_len, new_x_max)) + 1
            ...
            # 遍历数据，找到视野内的最高和最低价
            # (虽然这里遍历了所有数据，但Python处理3000条数据非常快，不会卡顿)
            for klu in current_meta.klu_iter():
                if klu.idx >= i_start and klu.idx < i_end:
                    if klu.high > y_max: y_max = klu.high
                    if klu.low < y_min: y_min = klu.low
                    has_data = True
                elif klu.idx >= i_end:
                    break # 数据是有序的，超出范围直接停止
            
            # 如果找到了有效的高低点，设置 Y 轴
            if has_data and y_max > y_min:
                padding = (y_max - y_min) * 0.05 # 上下留 5% 的边距，不让K线顶格
